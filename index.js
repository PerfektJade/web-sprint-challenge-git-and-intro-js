...
// console.log('Task 6:', addArtist(artists, "Jade", "1990-2022", "Full Stack Development", "Caucasian", "Lorem ipsum dolor sit amet labore et dolore magna aliqua. Ut enim ad minim veniam aliquip ex ea commodo consequat."));
...
function lotsOfArt(array) {
  return array.filter((e) => e.paintings > 100).map(({name}) => (name));
}
...
function artistInfo(array, artistName){
  return array.filter((e) => e.name === artistName).map(({bio}) => (bio)).toString();
}
...
function artistByCountry(array, artistNationality){
  return array.filter((e) => e.nationality === artistNationality).map(({name}) => (name));
}
